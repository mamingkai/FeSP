...
data_choice <- 7  # experimental dataset
sens_factor <- 2
...
```{r}
DATA_7383r5 <- DATA <- getExpData(data_choice)[1:36,]   # load experimental data
...
residual_sum_of_squares <- sum(na.omit(residuls_S^2))+sum(na.omit(residuls_P^2))
...
N.iter = 2000
wvar0 = NULL
MCMC_7383r5 <- MCMC <- fit_MCMC(PFit=PFit, pMin_MCMC=pMin_MCMC, pMax_MCMC=pMax_MCMC, OBS=DATA, N.iter,wvar0,updatecov = 1000,burninlength = 200,jump=NULL,covscale = Covar)
beep(sound = 2)
```
...
```{r plots}
summary_sR_7383r5 <- as.data.frame(summary(sR))
plot_sens_S <- fun_plot_sens_SP(summary_sR_7383r5,obs = DATA_7383r5,species = "S")+theme_bw()
plot_sens_P <- fun_plot_sens_SP(summary_sR_7383r5,obs = DATA_7383r5,"P")+theme_bw()

plot_sens_conc <- fun_plot_sens_others(df = summary_sR_7383r5,species = c("Fe","Sads", "Pads.fast","Pads.slow"),num_cols = 2,plot=F)
plot_sens_rates <- fun_plot_sens_others(df = summary_sR_7383r5,species = c("R.Fe","R.Pads", "R.Pdes.fast","R.Pdes.slow","R.Pdes.stoi","RHS.fast","RHS.slow","R.Sads","R.Sdes"),num_cols = 3,plot=F)
...
plot_sens_S_7383r5 <- plot_sens_S
plot_sens_P_7383r5 <- plot_sens_P
plot_sens_conc_combine_7383r5 <- cowplot::plot_grid(plotlist = plot_sens_conc, nrow = 2)
plot_sens_rates_combine_7383r5 <- cowplot::plot_grid(plotlist = plot_sens_rates, nrow = 3)

plot_sens_S_7383r5
plot_sens_P_7383r5
plot_sens_conc_combine_7383r5
plot_sens_rates_combine_7383r5
beep(sound=4)
```
```{r}
```
