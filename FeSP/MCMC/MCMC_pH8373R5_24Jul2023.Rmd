...
data_choice <- 9  # experimental dataset
sens_factor <- 2
# define fitting parameters (the others remain fixed, including their min/max range), here the trick used is to arbitrary define the best fit and pass it to the model
pIni <- c( log_kFe = -2.3,log_kS.fast= -0.8,log_kSads = 0,log_kPads=-4)
...
```{r}
DATA_8373r5 <-DATA <- getExpData(data_choice)  # load experimental data
...
```{r plots}
summary_sR_8373r5 <- as.data.frame(summary(sR))
plot_sens_S <- fun_plot_sens_SP(summary_sR_8373r5,obs = DATA_8373r5,species = "S")+theme_bw()
plot_sens_P <- fun_plot_sens_SP(summary_sR_8373r5,obs = DATA_8373r5,"P")+theme_bw()

plot_sens_conc <- fun_plot_sens_others(df = summary_sR_8373r5,species = c("Fe","Sads", "Pads.fast","Pads.slow"),num_cols = 2,plot=F)
plot_sens_rates <- fun_plot_sens_others(df = summary_sR_8373r5,species = c("R.Fe","R.Pads", "R.Pdes.fast","R.Pdes.slow","R.Pdes.stoi","RHS.fast","RHS.slow","R.Sads","R.Sdes"),num_cols = 3,plot=F)
...
plot_sens_S_8373r5 <- plot_sens_S
plot_sens_P_8373r5 <- plot_sens_P
plot_sens_conc_combine_8373r5 <- cowplot::plot_grid(plotlist = plot_sens_conc, nrow = 2)
plot_sens_rates_combine_8373r5 <- cowplot::plot_grid(plotlist = plot_sens_rates, nrow = 3)

plot_sens_S_8373r5
plot_sens_P_8373r5
plot_sens_conc_combine_8373r5
plot_sens_rates_combine_8373r5
beep(sound=4)
```
```{r}
```
