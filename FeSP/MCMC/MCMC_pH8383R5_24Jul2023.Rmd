...
data_choice <- 11  # experimental dataset
sens_factor <- 2
...
pIni <- c( log_kFe = -2.7,log_kS.fast= -0.73,log_kS.slow = -2.7,x=0.4)
...
```{r}
DATA_8383r5 <- DATA <- getExpData(data_choice)  # load experimental data
pars[names(pIni)] <- pIni            # ensure that all parameters are updated
outtimes <- DATA[,"time"]              # define outtimes based on exp. data
```

Next, we plot the experimental data together with the model prediction
to verify whether the guessed model parameters are "close enough".
...
Next, we perform the "simple" and MCMC fitting. Note that this step may
take a while depending on the number of iterations in the MCMC method.
...
N.iter = 2000
wvar0 = NULL
MCMC_8383r5 <- MCMC <- fit_MCMC(PFit=PFit, pMin_MCMC=pMin_MCMC, pMax_MCMC=pMax_MCMC, OBS=DATA, N.iter,wvar0,updatecov = 2000,burninlength = 200,jump=NULL,covscale = Covar)
beep(sound = 2)
```
...
```{r plots}
summary_sR_8383r5 <- as.data.frame(summary(sR))
plot_sens_S <- fun_plot_sens_SP(summary_sR_8383r5,obs = DATA_8383r5,species = "S")+theme_bw()
plot_sens_P <- fun_plot_sens_SP(summary_sR_8383r5,obs = DATA_8383r5,"P")+theme_bw()

plot_sens_conc <- fun_plot_sens_others(df = summary_sR_8383r5,species = c("Fe","Sads", "Pads.fast","Pads.slow"),num_cols = 2,plot=F)
plot_sens_rates <- fun_plot_sens_others(df = summary_sR_8383r5,species = c("R.Fe","R.Pads", "R.Pdes.fast","R.Pdes.slow","R.Pdes.stoi","RHS.fast","RHS.slow","R.Sads","R.Sdes"),num_cols = 3,plot=F)
...
plot_sens_S_8383r5 <- plot_sens_S
plot_sens_P_8383r5 <- plot_sens_P
plot_sens_conc_combine_8383r5 <- cowplot::plot_grid(plotlist = plot_sens_conc, nrow = 2)
plot_sens_rates_combine_8383r5 <- cowplot::plot_grid(plotlist = plot_sens_rates, nrow = 3)

plot_sens_S_8383r5
plot_sens_P_8383r5
plot_sens_conc_combine_8383r5
plot_sens_rates_combine_8383r5
beep(sound=4)
```
```{r}
```
